(function execute(inputs, outputs) {
  ...
  var list = inputs.script_availableSoftware.toString();

  array = list.split(',');

  var catalogItem = gs.getProperty('software_application_name'); // catalog item sys_id

  for (var i = 1; i < array.length; i++) {
    var grRitm = new GlideRecord('sc_req_item');
    grRitm.initialize();
    grRitm.cat_item = catalogItem;
    grRitm.request = inputs.script_request.sys_id;
    ...
    for (var j = 0; j < itemVarList.length; j++) {
      ...
      if (itemOption.item_option_new == gs.getProperty('candidate_name')) { // sys_id of candidate

        itemOption.value = inputs.script_candidatename.toString();
        itemOption.order = 1;
      }
      if (itemOption.item_option_new == gs.getProperty('manager_name')) { // sys_id of Manager

        itemOption.value = inputs.script_manager.toString();
        itemOption.order = 2;
      }

      if (itemOption.item_option_new == gs.getProperty('hire_date')) { // sys_id of Hire date
    ...
    if (itemOption.item_option_new == gs.getProperty('candidate_application')) { // sys_id of available software
  ...
  (function() {
    ...
    while (ritmGR.next()) {
      try {
        ...
        var result = sn_fd.FlowAPI.getRunner().flow('sn_hr_core.csodmultiple_software_application_checklist').inForeground().withInputs(inputs).run();
  ...
  ritmGRP.addQuery('sys_id', inputs.script_sysID);
  ritmGRP.query();
  if (ritmGRP.next()) {
